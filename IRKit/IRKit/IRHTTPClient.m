...
+ (IRHTTPClient *)waitForSignalWithCompletion:(void (^)(NSHTTPURLResponse *res, IRSignal *signal, NSError *error))completion {
    ...
    __unsafe_unretained IRHTTPClient *_client = client;
...
+ (void)showAlertOfError:(NSError *)error {
    ...
    if ([error.domain isEqualToString: NSURLErrorDomain]) {
        ...
        case NSURLErrorNotConnectedToInternet:
            // -1009
            message = @"インターネット接続を確認してください(-1009)";
            break;
        case NSURLErrorTimedOut:
            // -1001
            message = @"インターネット接続を確認してください(-1001)";
            break;
        default:
            break;
        }
        // IRLocalizedString(@"-1009 Please check your internet connection", @"-1009 error message")
        // IRLocalizedString(@"-1001 Please check your internet connection", @"-1001 error message")
    }
        ...
        if (error.userInfo && error.userInfo[@"message"]) {
            ...
            switch (error.code) {
            case 400:
                message = @"不正なリクエストです(400)";
                break;
            case 401:
                message = @"認証エラーが起こりました(401)";
                break;
            case 500:
                message = @"エラーが発生しました。もう一度お試しください(500)";
                break;
            case 503:
                message = @"ただいまメンテナンス中です。しばらくしてからもう一度お試しください(503)";
                break;
            default:
                break;
            }
            // IRLocalizedString(@"400 Invalid Request", @"http status code 400 error message")
            // IRLocalizedString(@"401 Unauthorized", @"http status code 401 error message")
            // IRLocalizedString(@"500 Something wrong, please try again, or contact us if problem persists", @"http status code 500 error message")
            // IRLocalizedString(@"503 We're temporary under maintenance, please wait for a while and try again", @"http status code 503 error message")
        }
    }
