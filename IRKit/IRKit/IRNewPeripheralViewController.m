...
- (void)viewDidLoad {
    ...
    __unsafe_unretained IRNewPeripheralViewController *_self = self;
...
- (void)searcher:(IRSearcher *)searcher didResolveService:(NSNetService *)service {
    ...
    if (!p.deviceid) {
        ...
        [p getKeyWithCompletion:^{
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle: @"IRKitを発見しました"
                                                            message: @""
                                                           delegate: self
...
- (void)wifiEditViewController:(IRWifiEditViewController *)viewController didFinishWithInfo:(NSDictionary *)info {
    ...
    if (!_keys.keysAreSet) {
#ifndef TARGET_IS_EXTENSION
        [[[UIAlertView alloc] initWithTitle: @"インターネット接続を確認してください。はじめに家のWi-Fiに接続してください"
                                    message: @""
                                   delegate: nil
                          cancelButtonTitle: @"OK"
                          otherButtonTitles: nil] show];
        // IRLocalizedString(@"Check your internet connection. Connect to your home Wi-Fi network first.", @"alert view title when ! keysAreSet")
#endif
        [self.delegate newPeripheralViewController: self
                           didFinishWithPeripheral: nil];
        return;
