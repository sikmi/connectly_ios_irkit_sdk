...
- (void)viewDidLoad {
    ...
    self.title = @"名前を設定";
    //IRLocalizedString(@"Give a name", @"title of IRPeripheralNameEdit");
...
- (void)finishInput {
    NSString *text = [NSString stringWithString:_peripheral.customizedName];
    ...
    [self.delegate nameEditViewController: self
                        didFinishWithInfo: @{
                                             ...
                                             IRViewControllerResultText: text
                                             }];
}
